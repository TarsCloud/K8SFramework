...
# FROM golang:1.16-jessie AS igolong
# RUN apt update && apt install git -y

FROM php:7.4.26-apache-jessie AS iphp

# image debian:jessie had "ls bug", we use busybox ls instead
RUN rm -rf /bin/ls
...
FROM openjdk:8-jessie AS ijava

FROM node:lts-jessie AS inode
