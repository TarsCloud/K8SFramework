...
var _ = ginkgo.Describe("ttemplate", func() {
	...
	defaultTTLayout := &tarsCrdV1beta3.TTemplate{
	...
	cppTTLayout := &tarsCrdV1beta3.TTemplate{
		ObjectMeta: k8sMetaV1.ObjectMeta{
			Name:      "tt.cpp",
			Namespace: s.Namespace,
		},
	...
	ginkgo.BeforeEach(func() {
		defaultTT, err := s.CRDClient.CrdV1beta3().TTemplates(s.Namespace).Create(context.TODO(), defaultTTLayout, k8sMetaV1.CreateOptions{})
	...
	ginkgo.It("check labels", func() {
		defaultTT, err := s.CRDClient.CrdV1beta3().TTemplates(s.Namespace).Get(context.TODO(), defaultTTLayout.Name, k8sMetaV1.GetOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), defaultTT)

		if defaultTT.Labels != nil {
			_, ok := defaultTT.Labels[tarsMetaV1beta3.ParentLabel]
			assert.False(ginkgo.GinkgoT(), ok)
		}

		cppTT, err := s.CRDClient.CrdV1beta3().TTemplates(s.Namespace).Get(context.TODO(), cppTTLayout.Name, k8sMetaV1.GetOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), defaultTT)
		exceptedCppTTLabels := map[string]string{
			tarsMetaV1beta3.ParentLabel: "tt.default",
		}
	...
	ginkgo.It("create ttemplate and parent not exist", func() {
		xxLayout := &tarsCrdV1beta3.TTemplate{
			ObjectMeta: k8sMetaV1.ObjectMeta{
				Name:      "tt.xxx",
				Namespace: s.Namespace,
			},
		...
		_, err := s.CRDClient.CrdV1beta3().TTemplates(s.Namespace).Create(context.TODO(), xxLayout, k8sMetaV1.CreateOptions{})
	...
	ginkgo.It("update ttemplate content", func() {
		...
		defaultXX, err := s.CRDClient.CrdV1beta3().TTemplates(s.Namespace).Patch(context.TODO(), defaultTTLayout.Name, patchTypes.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), defaultXX)
		assert.True(ginkgo.GinkgoT(), defaultXX.Spec.Content == "new content")

		cppTT, err := s.CRDClient.CrdV1beta3().TTemplates(s.Namespace).Patch(context.TODO(), cppTTLayout.Name, patchTypes.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), cppTT)
	...
	ginkgo.Context("delete ttemplate ", func() {
		ginkgo.It("reference by another ttemplate ", func() {
			err := s.CRDClient.CrdV1beta3().TTemplates(s.Namespace).Delete(context.TODO(), defaultTTLayout.Name, k8sMetaV1.DeleteOptions{})
			assert.NotNil(ginkgo.GinkgoT(), err)
		})
		...
		ginkgo.It("reference by another tserver", func() {
			...
			err = s.CRDClient.CrdV1beta3().TTemplates(s.Namespace).Delete(context.TODO(), cppTTLayout.Name, k8sMetaV1.DeleteOptions{})
			assert.NotNil(ginkgo.GinkgoT(), err)
		})
