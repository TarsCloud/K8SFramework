...
var _ = ginkgo.Describe("test timage", func() {
	...
	ginkgo.It("try create timage", func() {
		...
		timage, err := s.CRDClient.AppsV1beta2().TImages(s.Namespace).Create(context.TODO(), tiLayout, k8sMetaV1.CreateOptions{})
	...
	ginkgo.It("try create/update timage with duplicate id ", func() {
		...
		timage, err = s.CRDClient.AppsV1beta2().TImages(s.Namespace).Patch(context.TODO(), "test-testserver", patchTypes.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), timage)
