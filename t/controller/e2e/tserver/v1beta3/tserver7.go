...
var _ = ginkgo.Describe("try create/update normal server and check statefulset", func() {
	...
	ginkgo.It("podManagementPolicy", func() {
		tserver, _ := tarsRuntime.Clients.CrdClient.TarsV1beta3().TServers(s.Namespace).Get(context.TODO(), Resource, k8sMetaV1.GetOptions{})
		...
		bs, _ := json.Marshal(jsonPatch)
		tserver, err := tarsRuntime.Clients.CrdClient.TarsV1beta3().TServers(s.Namespace).Patch(context.TODO(), Resource, patchTypes.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), tserver)
	...
	ginkgo.Context("readinessGate", func() {
		...
		ginkgo.It("new readinessGate", func() {
			newReadiesGate1 := scaffold.RandStringRunes(10)
			newReadiesGate2 := scaffold.RandStringRunes(10)
			newReadiesGate3 := scaffold.RandStringRunes(10)
			jsonPatch := tarsTool.JsonPatch{
				{
					OP:    tarsTool.JsonPatchAdd,
					Path:  "/spec/k8s/readinessGates",
					Value: []string{newReadiesGate1, newReadiesGate2, newReadiesGate3},
				},
			}
			bs, _ := json.Marshal(jsonPatch)
			_, err := tarsRuntime.Clients.CrdClient.TarsV1beta3().TServers(s.Namespace).Patch(context.TODO(), Resource, patchTypes.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
			...
			expectedReadinessGates := []k8sCoreV1.PodReadinessGate{
				{
					ConditionType: k8sCoreV1.PodConditionType(newReadiesGate1),
				},
				{
					ConditionType: k8sCoreV1.PodConditionType(newReadiesGate2),
				},
				{
					ConditionType: k8sCoreV1.PodConditionType(newReadiesGate3),
				},
			}
