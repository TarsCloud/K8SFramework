...
var _ = ginkgo.Describe("test server level config", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		tconfig, err := tarsRuntime.Clients.CrdClient.AppsV1beta2().TConfigs(s.Namespace).Create(context.TODO(), tconfigLayout, k8sMetaV1.CreateOptions{})
	...
	ginkgo.Context("slave tconfig", func() {
		...
		ginkgo.It("create/delete slave tconfig", func() {
			...
			slaveTConfig, err := tarsRuntime.Clients.CrdClient.AppsV1beta2().TConfigs(s.Namespace).Create(context.TODO(), slaveTConfigLayout, k8sMetaV1.CreateOptions{})
			assert.Nil(ginkgo.GinkgoT(), err)
			...
			err = tarsRuntime.Clients.CrdClient.AppsV1beta2().TConfigs(s.Namespace).Delete(context.TODO(), ResourceName, k8sMetaV1.DeleteOptions{})
			assert.NotNil(ginkgo.GinkgoT(), err)

			err = tarsRuntime.Clients.CrdClient.AppsV1beta2().TConfigs(s.Namespace).Delete(context.TODO(), slaveResourceName, k8sMetaV1.DeleteOptions{})
			assert.Nil(ginkgo.GinkgoT(), err)
			time.Sleep(s.Opts.SyncTime)
