...
var _ = ginkgo.Describe("test account", func() {
	...
	ginkgo.BeforeEach(func() {
		taccountLayout := &tarsV1Beta2.TAccount{
			TypeMeta: k8sMetaV1.TypeMeta{},
			ObjectMeta: k8sMetaV1.ObjectMeta{
				Name:      ResourceName,
				Namespace: s.Namespace,
			},
			Spec: tarsV1Beta2.TAccountSpec{
				Username: Username,
				Authentication: tarsV1Beta2.TAccountAuthentication{
					Password:  &Password,
					Activated: true,
				},
				Authorization: []*tarsV1Beta2.TAccountAuthorization{},
			},
		}
		var taccount *tarsV1Beta2.TAccount
		taccount, err := tarsRuntime.Clients.CrdClient.TarsV1beta2().TAccounts(s.Namespace).Create(context.TODO(), taccountLayout, k8sMetaV1.CreateOptions{})
	...
	ginkgo.It("check bcrypt password ", func() {
		taccount, err := tarsRuntime.Clients.CrdClient.TarsV1beta2().TAccounts(s.Namespace).Get(context.TODO(), ResourceName, k8sMetaV1.GetOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), taccount)
	...
	ginkgo.It("try update password", func() {
		...
		for i := 0; i < updateTimes; i++ {
			...
			tokensPatch := tarsTool.JsonPatch{
				{
					OP:   tarsTool.JsonPatchAdd,
					Path: "/spec/authentication/tokens",
					Value: []tarsV1Beta2.TAccountAuthenticationToken{
						{
							Name:           "v1",
			...
			taccount, err = tarsRuntime.Clients.CrdClient.TarsV1beta2().TAccounts(s.Namespace).Patch(context.TODO(), ResourceName, patchTypes.JSONPatchType, tokensPatchContent, k8sMetaV1.PatchOptions{})
			assert.Nil(ginkgo.GinkgoT(), err)
			assert.NotNil(ginkgo.GinkgoT(), taccount)
			...
			taccount, err = tarsRuntime.Clients.CrdClient.TarsV1beta2().TAccounts(s.Namespace).Patch(context.TODO(), ResourceName, patchTypes.JSONPatchType, patchContent, k8sMetaV1.PatchOptions{})
			assert.Nil(ginkgo.GinkgoT(), err)
			assert.NotNil(ginkgo.GinkgoT(), taccount)
			assert.Nil(ginkgo.GinkgoT(), taccount.Spec.Authentication.Password)
			assert.NotNil(ginkgo.GinkgoT(), taccount.Spec.Authentication.BCryptPassword)
