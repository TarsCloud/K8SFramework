...
var _ = ginkgo.Describe("test account", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		taccount, err := s.CRDClient.AppsV1beta2().TAccounts(s.Namespace).Create(context.TODO(), taccountLayout, k8sMetaV1.CreateOptions{})
	...
	ginkgo.It("check bcrypt password ", func() {
		taccount, err := s.CRDClient.AppsV1beta2().TAccounts(s.Namespace).Get(context.TODO(), ResourceName, k8sMetaV1.GetOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), taccount)
	...
	ginkgo.It("try update password", func() {
		...
		for i := 0; i < updateTimes; i++ {
			...
			taccount, err = s.CRDClient.AppsV1beta2().TAccounts(s.Namespace).Patch(context.TODO(), ResourceName, patchTypes.JSONPatchType, tokensPatchContent, k8sMetaV1.PatchOptions{})
			assert.Nil(ginkgo.GinkgoT(), err)
			assert.NotNil(ginkgo.GinkgoT(), taccount)
			...
			taccount, err = s.CRDClient.AppsV1beta2().TAccounts(s.Namespace).Patch(context.TODO(), ResourceName, patchTypes.JSONPatchType, patchContent, k8sMetaV1.PatchOptions{})
			assert.Nil(ginkgo.GinkgoT(), err)
			assert.NotNil(ginkgo.GinkgoT(), taccount)
			assert.Nil(ginkgo.GinkgoT(), taccount.Spec.Authentication.Password)
			assert.NotNil(ginkgo.GinkgoT(), taccount.Spec.Authentication.BCryptPassword)
