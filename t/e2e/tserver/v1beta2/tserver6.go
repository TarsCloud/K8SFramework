				...
				RequiredDuringSchedulingIgnoredDuringExecution: &k8sCoreV1.NodeSelector{NodeSelectorTerms: []k8sCoreV1.NodeSelectorTerm{
					{
						MatchExpressions: []k8sCoreV1.NodeSelectorRequirement{
							{
								Key:      fmt.Sprintf("%s.%s", crdMeta.TarsNodeLabel, s.Namespace),
					...
					NodeSelectorTerms: []k8sCoreV1.NodeSelectorTerm{
						{
							MatchExpressions: []k8sCoreV1.NodeSelectorRequirement{
								...
								{
									Key:      fmt.Sprintf("%s.%s.%s", crdMeta.TarsAbilityLabelPrefix, s.Namespace, App),
									Operator: k8sCoreV1.NodeSelectorOpExists,
								},
							},
						},
			...
			jsonPatch := crdMeta.JsonPatch{
				{
					OP:    crdMeta.JsonPatchReplace,
					Path:  "/spec/k8s/abilityAffinity",
					Value: crdV1Beta2.ServerRequired,
				},
			}
			bs, _ := json.Marshal(jsonPatch)
			_, err := s.CRDClient.CrdV1beta2().TServers(s.Namespace).Patch(context.TODO(), Resource, patchTypes.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
				...
				RequiredDuringSchedulingIgnoredDuringExecution: &k8sCoreV1.NodeSelector{NodeSelectorTerms: []k8sCoreV1.NodeSelectorTerm{
					{
						MatchExpressions: []k8sCoreV1.NodeSelectorRequirement{
							...
							{
								Key:      fmt.Sprintf("%s.%s.%s-%s", crdMeta.TarsAbilityLabelPrefix, s.Namespace, App, Server),
								Operator: k8sCoreV1.NodeSelectorOpExists,
							},
						},
					},
			...
			jsonPatch := crdMeta.JsonPatch{
				{
					OP:    crdMeta.JsonPatchReplace,
					Path:  "/spec/k8s/abilityAffinity",
					Value: crdV1Beta2.AppOrServerPreferred,
				},
			}
			bs, _ := json.Marshal(jsonPatch)
			_, err := s.CRDClient.CrdV1beta2().TServers(s.Namespace).Patch(context.TODO(), Resource, patchTypes.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
	...
	ginkgo.Context("readinessGate", func() {
		...
		ginkgo.It("new readinessGate", func() {
			...
			assert.True(ginkgo.GinkgoT(), func() bool {
				a, b := false, false
				for i := range spec.ReadinessGates {
					if string(spec.ReadinessGates[i].ConditionType) == crdMeta.TPodReadinessGate {
						a = true
					}

					if string(spec.ReadinessGates[i].ConditionType) == newReadiesGate {
						b = true
					}
				}
				return a && b
