...
var _ = ginkgo.Describe("test account", func() {
	...
	ginkgo.BeforeEach(func() {
		taccountLayout := &crdV1Beta1.TAccount{
			TypeMeta: k8sMetaV1.TypeMeta{},
			ObjectMeta: k8sMetaV1.ObjectMeta{
				Name:      ResourceName,
				Namespace: s.Namespace,
			},
			Spec: crdV1Beta1.TAccountSpec{
				Username: Username,
				Authentication: crdV1Beta1.TAccountAuthentication{
					Password:  &Password,
					Activated: true,
				},
				Authorization: []*crdV1Beta1.TAccountAuthorization{},
			},
		}
		var taccount *crdV1Beta1.TAccount
		taccount, err := s.CRDClient.CrdV1beta1().TAccounts(s.Namespace).Create(context.TODO(), taccountLayout, k8sMetaV1.CreateOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), taccount)
	})

	ginkgo.It("check bcrypt password ", func() {
		taccount, err := s.CRDClient.CrdV1beta1().TAccounts(s.Namespace).Get(context.TODO(), ResourceName, k8sMetaV1.GetOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), taccount)
	...
	ginkgo.It("try update password", func() {
		...
		for i := 0; i < updateTimes; i++ {
			...
			tokensPatch := crdMeta.JsonPatch{
				{
					...
					Value: []crdV1Beta1.TAccountAuthenticationToken{
						{
							Name:           "v1",
			...
			taccount, err = s.CRDClient.CrdV1beta1().TAccounts(s.Namespace).Patch(context.TODO(), ResourceName, patchTypes.JSONPatchType, tokensPatchContent, k8sMetaV1.PatchOptions{})
			assert.Nil(ginkgo.GinkgoT(), err)
			assert.NotNil(ginkgo.GinkgoT(), taccount)
			...
			taccount, err = s.CRDClient.CrdV1beta1().TAccounts(s.Namespace).Patch(context.TODO(), ResourceName, patchTypes.JSONPatchType, patchContent, k8sMetaV1.PatchOptions{})
			assert.Nil(ginkgo.GinkgoT(), err)
			assert.NotNil(ginkgo.GinkgoT(), taccount)
			assert.Nil(ginkgo.GinkgoT(), taccount.Spec.Authentication.Password)
			assert.NotNil(ginkgo.GinkgoT(), taccount.Spec.Authentication.BCryptPassword)
		...
		patchContent, _ := json.Marshal(jsonPatch)
		taccount, _ = s.CRDClient.CrdV1beta1().TAccounts(s.Namespace).Patch(context.TODO(), ResourceName, patchTypes.JSONPatchType, patchContent, k8sMetaV1.PatchOptions{})
		assert.NotNil(ginkgo.GinkgoT(), taccount)
		assert.Nil(ginkgo.GinkgoT(), taccount.Spec.Authentication.Password)
		assert.NotNil(ginkgo.GinkgoT(), taccount.Spec.Authentication.BCryptPassword)
		sha1String := fmt.Sprintf("%x", sha1.Sum([]byte(Password)))
