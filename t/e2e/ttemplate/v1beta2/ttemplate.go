...
var _ = ginkgo.Describe("test app level config", func() {
	...
	ginkgo.It("check labels", func() {
		defaultTT, err := s.CRDClient.CrdV1beta2().TTemplates(s.Namespace).Get(context.TODO(), "tt.default", k8sMetaV1.GetOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), defaultTT)
		...
		cppTT, err := s.CRDClient.CrdV1beta2().TTemplates(s.Namespace).Get(context.TODO(), "tt.cpp", k8sMetaV1.GetOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), defaultTT)
	...
	ginkgo.It("try update ttemplate content", func() {
		...
		defaultXX, err := s.CRDClient.CrdV1beta2().TTemplates(s.Namespace).Patch(context.TODO(), "tt.default", patchTypes.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), defaultXX)
		assert.True(ginkgo.GinkgoT(), defaultXX.Spec.Content == "new content")

		cppTT, err := s.CRDClient.CrdV1beta2().TTemplates(s.Namespace).Patch(context.TODO(), "tt.cpp", patchTypes.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		assert.NotNil(ginkgo.GinkgoT(), cppTT)
	...
	ginkgo.It("try delete ttemplate ", func() {
		err := s.CRDClient.CrdV1beta2().TTemplates(s.Namespace).Delete(context.TODO(), "tt.default", k8sMetaV1.DeleteOptions{})
		assert.NotNil(ginkgo.GinkgoT(), err)
		...
		tsLayout := &crdV1Beta2.TServer{
			...
			Spec: crdV1Beta2.TServerSpec{
				...
				Release: &crdV1Beta2.TServerRelease{
					...
					TServerReleaseNode: &crdV1Beta2.TServerReleaseNode{
						Image:  "xxxx.com/tarsnode:v1",
						Secret: "",
					},
				},
			},
		...
		err = s.CRDClient.CrdV1beta2().TTemplates(s.Namespace).Delete(context.TODO(), "tt.cpp", k8sMetaV1.DeleteOptions{})
		assert.NotNil(ginkgo.GinkgoT(), err)

		err = s.CRDClient.CrdV1beta2().TServers(s.Namespace).Delete(context.TODO(), "test-testserver", k8sMetaV1.DeleteOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
