...
var _ = ginkgo.Describe("test create slave config before master", func() {
	...
	var err error
	_, err = s.CRDClient.CrdV1beta2().TConfigs(s.Namespace).Create(context.TODO(), appLevelSlaveLayout, k8sMetaV1.CreateOptions{})
	assert.NotNil(ginkgo.GinkgoT(), err)

	_, err = s.CRDClient.CrdV1beta2().TConfigs(s.Namespace).Create(context.TODO(), serverLevelSlaveLayout, k8sMetaV1.CreateOptions{})
	assert.NotNil(ginkgo.GinkgoT(), err)
})
...
var _ = ginkgo.Describe("test app level master/slaver config", func() {
	...
	ginkgo.BeforeEach(func() {
		masterTConfig, err := s.CRDClient.CrdV1beta2().TConfigs(s.Namespace).Create(context.TODO(), master, k8sMetaV1.CreateOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
		...
		slaveTConfig, err := s.CRDClient.CrdV1beta2().TConfigs(s.Namespace).Create(context.TODO(), slave, k8sMetaV1.CreateOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
	...
	ginkgo.It("try delete app level tconfig", func() {
		err := s.CRDClient.CrdV1beta2().TConfigs(s.Namespace).Delete(context.TODO(), MasterResourceName, k8sMetaV1.DeleteOptions{})
		assert.NotNil(ginkgo.GinkgoT(), err)

		err = s.CRDClient.CrdV1beta2().TConfigs(s.Namespace).Delete(context.TODO(), SlaverResourceName, k8sMetaV1.DeleteOptions{})
		assert.Nil(ginkgo.GinkgoT(), err)
