...
func (dk *BasicDockerKeyring) Add(cfg DockerConfig) {
	...
	for loc, ident := range cfg {
		...
		if creds.Username == "" || creds.Password == "" {
			creds.Username, creds.Password = decodeDockerConfigFieldAuth(creds.Auth)
		}
...
func decodeDockerConfigFieldAuth(field string) (username, password string) {
	var decoded []byte
	...
	if strings.HasSuffix(strings.TrimSpace(field), "=") {
		...
		decoded, _ = base64.RawStdEncoding.DecodeString(field)
	}
