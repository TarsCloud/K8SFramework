...
func prepareActiveTConfig(newTConfig *tarsV1beta3.TConfig, listers *informer.Listers) error {
	if !listers.TCSynced() {
	...
	_, err = tarsRuntime.Clients.CrdClient.TarsV1beta3().TConfigs(namespace).Patch(context.TODO(), names[0], patchTypes.JSONPatchType, patchContent, k8sMetaV1.PatchOptions{})
	if err != nil {
...
func prepareDeleteTConfig(tconfig *tarsV1beta3.TConfig, listers *informer.Listers) error {
	if !listers.TCSynced() {
		return fmt.Errorf("tconfig infomer has not finished syncing")
	}
...
func validCreateTConfig(listers *informer.Listers, view *k8sAdmissionV1.AdmissionReview) error {
	newTConfig := &tarsV1beta3.TConfig{}
...
func validUpdateTConfig(listers *informer.Listers, view *k8sAdmissionV1.AdmissionReview) error {
	...
	oldTConfig := &tarsV1beta3.TConfig{}
