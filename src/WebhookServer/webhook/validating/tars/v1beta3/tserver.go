...
func validTServer(newTServer, oldTServer *tarsV1beta3.TServer, listers *lister.Listers) error {
		...
		for i := range newTServer.Spec.K8S.Mounts {
			...
			if mount.Source.TLocalVolume != nil || mount.Source.PersistentVolumeClaimTemplate != nil {
				if newTServer.Spec.K8S.DaemonSet {
					return fmt.Errorf(tarsMeta.ResourceInvalidError, "tserver", fmt.Sprintf("can not use TLocalVolume and PersistentVolumeClaimTemplate when .daemonSet value is true"))
				}
			}
...
func init() {
	gvr := tarsV1beta3.SchemeGroupVersion.WithResource("tservers")
