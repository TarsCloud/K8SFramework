...
constexpr ssize_t UPDATE_STATE_INTERVAL = 420u; /* 上传服务状态间隔时间, 单位:秒 */
constexpr ssize_t ACTIVE_CHECK_INTERVAL = 1u;  /* 检测服务是否启动的间隔时间, 单位:秒 */
constexpr ssize_t INACTIVE_CHECK_INTERVAL = 1u;  /* 检测服务是否停止的间隔时间, 单位:秒 */
constexpr ssize_t LAUNCH_SERVER_INTERVAL_TIME = 30u;    /* 启动服务间隔时间, 单位:秒 */

constexpr pid_t MIN_PID_VALUE = 2;    /* 最小的合法 pid 值 */

constexpr ssize_t MAX_TERM_TIME = 10u;  /* 向业务进程发送 AminObj->async_shutdown() 或 发送 SIGTERM信号后的等待时间,若超时后Pid仍然存在,则发送SIGKILL信号, 单位:秒 */

constexpr ssize_t MAX_KILL_TIME = 3u;   /* 向业务进程发送 SIGKILL 信号后的等待时间, 若超时后Pid仍然存在,则tarsnode退出, 单位:秒 */
