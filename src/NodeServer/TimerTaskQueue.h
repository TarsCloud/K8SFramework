...
public:
    ...
    void pushCycleTask(const std::function<void(const size_t &callTimes, size_t &nextTimer)> &t, size_t first,
                       size_t cycle) {
        size_t callTimes = 0;
...
private:
    void _runCycleTask(const std::function<void(const size_t &callTimes, size_t &nextTimer)> &t, size_t callTimes,
                       size_t cycle) {
        t(callTimes, cycle);
        if (cycle != 0) {
            callTimes += 1;
