...
func (r *TConfigReconciler) reconcile(key string) reconclie.ReconcileResult {
	...
	err := r.clients.CrdClient.CrdV1alpha2().TConfigs(namespace).DeleteCollection(context.TODO(), k8sMetaV1.DeleteOptions{}, k8sMetaV1.ListOptions{
	...
	for _, tconfig := range tconfigs {
		...
		_, err = r.clients.CrdClient.CrdV1alpha2().TConfigs(namespace).Patch(context.TODO(), name, patchTypes.JSONPatchType, patchContent, k8sMetaV1.PatchOptions{})
		if err != nil {
