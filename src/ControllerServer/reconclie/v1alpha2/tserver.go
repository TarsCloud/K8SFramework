...
func (r *TServerReconciler) reconcile(key string) reconclie.ReconcileResult {
	...
	tserverCopy.Status = crdV1alpha2.TServerStatus{
		Selector:        selector.String(),
		Replicas:        *tserver.Spec.K8S.Replicas,
		ReadyReplicas:   readySize,
		CurrentReplicas: int32(len(pods)),
	}
	_, err = r.clients.CrdClient.CrdV1alpha2().TServers(namespace).UpdateStatus(context.TODO(), tserverCopy, k8sMetaV1.UpdateOptions{})
	if err != nil {
