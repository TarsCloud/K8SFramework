...
func (r *TExitedRecordReconciler) EnqueueObj(resourceName string, resourceEvent k8sWatchV1.EventType, resourceObj interface{}) {
	...
	case *crdV1beta1.TServer:
		...
		r.workQueue.Add(key)
	case *crdV1beta1.TExitedRecord:
		texitedRecord := resourceObj.(*crdV1beta1.TExitedRecord)
	...
	case *k8sCoreV1.Pod:
		...
		if pod.DeletionTimestamp != nil && pod.UID != "" && pod.Labels != nil {
			...
			if appExist && serverExist {
				tExitedEvent := &crdV1beta1.TExitedRecord{
					App:    app,
					Server: server,
...
func (r *TExitedRecordReconciler) reconcileBaseTServer(namespace string, name string) reconclie.ReconcileResult {
	...
	if err != nil {
		...
		err = r.clients.CrdClient.CrdV1beta1().TExitedRecords(namespace).Delete(context.TODO(), name, k8sMetaV1.DeleteOptions{})
	...
	if err != nil {
		...
		tExitedPodInterface := r.clients.CrdClient.CrdV1beta1().TExitedRecords(namespace)
...
func (r *TExitedRecordReconciler) reconcileBasePod(namespace string, tExitedPodSpecString string) reconclie.ReconcileResult {
	var tExitedEvent crdV1beta1.TExitedRecord
	...
	_, err = r.clients.CrdClient.CrdV1beta1().TExitedRecords(namespace).Patch(context.TODO(), tExitedRecordName, patchTypes.JSONPatchType, []byte(patchString), k8sMetaV1.PatchOptions{})
