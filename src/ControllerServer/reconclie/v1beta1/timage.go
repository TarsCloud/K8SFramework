...
func (r *TImageReconciler) EnqueueObj(resourceName string, resourceEvent k8sWatchV1.EventType, resourceObj interface{}) {
	...
	case *crdV1beta1.TImage:

		timage := resourceObj.(*crdV1beta1.TImage)
...
func (r *TImageReconciler) reconcile(key string) reconclie.ReconcileResult {
	...
	buildState := crdV1beta1.TImageBuild{
		Last: &crdV1beta1.TImageBuildState{
	...
	_, err = r.clients.CrdClient.CrdV1beta1().TImages(namespace).Patch(context.TODO(), name, types.JSONPatchType, []byte(patchContent), k8sMetaV1.PatchOptions{})
