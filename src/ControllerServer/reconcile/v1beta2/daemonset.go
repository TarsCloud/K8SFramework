...
func (r *DaemonSetReconciler) reconcile(key string) reconcile.Result {
	...
	if err != nil {
		...
		if err != nil && !errors.IsNotFound(err) {
			utilRuntime.HandleError(fmt.Errorf(crdMeta.ResourceDeleteError, "daemonset", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
		return reconcile.AllOk
	}
	...
	if err != nil {
		if !errors.IsNotFound(err) {
			utilRuntime.HandleError(fmt.Errorf(crdMeta.ResourceGetError, "daemonset", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
		...
		if _, err = daemonSetInterface.Create(context.TODO(), daemonSet, k8sMetaV1.CreateOptions{}); err != nil && !errors.IsAlreadyExists(err) {
			utilRuntime.HandleError(fmt.Errorf(crdMeta.ResourceCreateError, "daemonset", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
	...
	if !k8sMetaV1.IsControlledBy(daemonSet, tserver) {
		...
		msg := fmt.Sprintf(crdMeta.ResourceOutControlError, "daemonset", namespace, daemonSet.Name, namespace, name)
	...
	if anyChanged {
		...
		if _, err := daemonSetInterface.Update(context.TODO(), daemonSetCopy, k8sMetaV1.UpdateOptions{}); err != nil {
			utilRuntime.HandleError(fmt.Errorf(crdMeta.ResourceUpdateError, "daemonset", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
	}
	return reconcile.AllOk
