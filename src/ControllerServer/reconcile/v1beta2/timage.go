...
func (r *TImageReconciler) EnqueueObj(resourceName string, resourceEvent k8sWatchV1.EventType, resourceObj interface{}) {
	...
	case *crdV1beta2.TImage:
		timage := resourceObj.(*crdV1beta2.TImage)
		...
		if timage.ImageType == "server" {
			if timage.Build != nil && timage.Build.Running != nil {
				maxBuildTime := crdMeta.DefaultMaxImageBuildTime
...
func (r *TImageReconciler) reconcile(key string) reconcile.Result {
	...
	if err != nil {
		...
		utilRuntime.HandleError(fmt.Errorf(crdMeta.ResourceGetError, "timage", namespace, name, err.Error()))
	...
	case reconcileTargetCheckImageBuildOvertime:
		...
		buildState := crdV1beta2.TImageBuild{
			Last: &crdV1beta2.TImageBuildState{
	...
	if jsonPatch != nil {
		...
		_, err = r.clients.CrdClient.CrdV1beta2().TImages(namespace).Patch(context.TODO(), name, types.JSONPatchType, bs, k8sMetaV1.PatchOptions{})
		if err != nil {
