...
func (r *ServiceReconciler) reconcile(key string) reconcile.Result {
	...
	if err != nil {
		...
		if err != nil && !errors.IsNotFound(err) {
			utilRuntime.HandleError(fmt.Errorf(crdMeta.ResourceDeleteError, "service", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
		return reconcile.AllOk
	}
	...
	if err != nil {
		if !errors.IsNotFound(err) {
			utilRuntime.HandleError(fmt.Errorf(crdMeta.ResourceGetError, "service", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
		...
		if _, err = serviceInterface.Create(context.TODO(), service, k8sMetaV1.CreateOptions{}); err != nil && !errors.IsAlreadyExists(err) {
			utilRuntime.HandleError(fmt.Errorf(crdMeta.ResourceCreateError, "service", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
	...
	if !k8sMetaV1.IsControlledBy(service, tserver) {
		...
		msg := fmt.Sprintf(crdMeta.ResourceOutControlError, "service", namespace, service.Name, namespace, name)
	...
	if anyChanged {
		...
		if _, err := serviceInterface.Update(context.TODO(), serviceCopy, k8sMetaV1.UpdateOptions{}); err != nil {
			utilRuntime.HandleError(fmt.Errorf(crdMeta.ResourceUpdateError, "service", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
	}
