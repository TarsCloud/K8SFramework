...
func (r *ServiceReconciler) reconcile(key string) reconcile.Result {
	...
	if err != nil {
		...
		if err != nil && !errors.IsNotFound(err) {
			utilRuntime.HandleError(fmt.Errorf(tarsMetaV1beta3.ResourceDeleteError, "service", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
		return reconcile.AllOk
	}
	...
	if err != nil {
		if !errors.IsNotFound(err) {
			utilRuntime.HandleError(fmt.Errorf(tarsMetaV1beta3.ResourceGetError, "service", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
		...
		if _, err = serviceInterface.Create(context.TODO(), service, k8sMetaV1.CreateOptions{}); err != nil && !errors.IsAlreadyExists(err) {
			utilRuntime.HandleError(fmt.Errorf(tarsMetaV1beta3.ResourceCreateError, "service", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
	...
	if !k8sMetaV1.IsControlledBy(service, tserver) {
		...
		msg := fmt.Sprintf(tarsMetaV1beta3.ResourceOutControlError, "service", namespace, service.Name, namespace, name)
	...
	if anyChanged {
		...
		if _, err := serviceInterface.Update(context.TODO(), serviceCopy, k8sMetaV1.UpdateOptions{}); err != nil {
			utilRuntime.HandleError(fmt.Errorf(tarsMetaV1beta3.ResourceUpdateError, "service", namespace, name, err.Error()))
			return reconcile.RateLimit
		}
	}
