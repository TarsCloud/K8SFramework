...
{
	std::string response{};
	if (ESClient::instance().doRequest(ESClientRequestMethod::Put, LogIndexUri, LogIndexTemplate, response) != 200)
	{
		TLOGERROR("create index template \"tars_call_log\" error: " << response);
		return;
	}
	if (ESClient::instance().doRequest(ESClientRequestMethod::Put, TraceIndexTemplateUri, TraceIndexTemplate, response) != 200)
	{
		TLOGERROR("create index template \"tars_call_trace\" error: " << response);
		return;
	}
	if (ESClient::instance().doRequest(ESClientRequestMethod::Put, GraphIndexTemplateUri, GraphIndexTemplate, response) != 200)
	{
