 ...
 */
	...
	Storage::instance().getTEndpoints(
			[&v, &activeEp, &inactiveEp, &serviceExistInCluster](const std::map<std::string, std::shared_ptr<TEndpoint>>& tendpoints) mutable
			{
...
Int32
QueryImp::findObjectByIdInSameSet(const std::string& id, const std::string& setId, vector<EndpointF>& activeEp, vector<EndpointF>& inactiveEp,
		CurrentPtr current)
{
	findObjectById_(id, activeEp, &inactiveEp);
	auto str = buildLogContent(current, activeEp, &inactiveEp);
