...
type TTemplateLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.TTemplate, err error)
...
// List lists all TTemplates in the indexer.
func (s *tTemplateLister) List(selector labels.Selector) (ret []*v1alpha2.TTemplate, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.TTemplate))
	})
	return ret, err
}
...
type TTemplateNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.TTemplate, error)
...
// List lists all TTemplates in the indexer for a given namespace.
func (s tTemplateNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.TTemplate, err error) {
...
// Get retrieves the TTemplate from the indexer for a given namespace and name.
func (s tTemplateNamespaceLister) Get(name string) (*v1alpha2.TTemplate, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("ttemplate"), name)
	}
	return obj.(*v1alpha2.TTemplate), nil
}
