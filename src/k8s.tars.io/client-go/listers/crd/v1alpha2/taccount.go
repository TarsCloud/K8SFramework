...
type TAccountLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.TAccount, err error)
...
// List lists all TAccounts in the indexer.
func (s *tAccountLister) List(selector labels.Selector) (ret []*v1alpha2.TAccount, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.TAccount))
	})
	return ret, err
}
...
type TAccountNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.TAccount, error)
...
// List lists all TAccounts in the indexer for a given namespace.
func (s tAccountNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.TAccount, err error) {
...
// Get retrieves the TAccount from the indexer for a given namespace and name.
func (s tAccountNamespaceLister) Get(name string) (*v1alpha2.TAccount, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("taccount"), name)
	}
	return obj.(*v1alpha2.TAccount), nil
}
