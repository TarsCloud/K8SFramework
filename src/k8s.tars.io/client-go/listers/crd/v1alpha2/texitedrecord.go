...
type TExitedRecordLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.TExitedRecord, err error)
...
// List lists all TExitedRecords in the indexer.
func (s *tExitedRecordLister) List(selector labels.Selector) (ret []*v1alpha2.TExitedRecord, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.TExitedRecord))
	})
	return ret, err
}
...
type TExitedRecordNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.TExitedRecord, error)
...
// List lists all TExitedRecords in the indexer for a given namespace.
func (s tExitedRecordNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.TExitedRecord, err error) {
...
// Get retrieves the TExitedRecord from the indexer for a given namespace and name.
func (s tExitedRecordNamespaceLister) Get(name string) (*v1alpha2.TExitedRecord, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("texitedrecord"), name)
	}
	return obj.(*v1alpha2.TExitedRecord), nil
}
