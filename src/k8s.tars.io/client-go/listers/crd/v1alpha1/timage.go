...
type TImageLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.TImage, err error)
...
// List lists all TImages in the indexer.
func (s *tImageLister) List(selector labels.Selector) (ret []*v1alpha1.TImage, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.TImage))
	})
	return ret, err
}
...
type TImageNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.TImage, error)
...
// List lists all TImages in the indexer for a given namespace.
func (s tImageNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.TImage, err error) {
...
// Get retrieves the TImage from the indexer for a given namespace and name.
func (s tImageNamespaceLister) Get(name string) (*v1alpha1.TImage, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("timage"), name)
	}
	return obj.(*v1alpha1.TImage), nil
}
