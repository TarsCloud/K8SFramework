...
type TEndpointLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta2.TEndpoint, err error)
...
// List lists all TEndpoints in the indexer.
func (s *tEndpointLister) List(selector labels.Selector) (ret []*v1beta2.TEndpoint, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta2.TEndpoint))
	})
	return ret, err
}
...
type TEndpointNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta2.TEndpoint, error)
...
// List lists all TEndpoints in the indexer for a given namespace.
func (s tEndpointNamespaceLister) List(selector labels.Selector) (ret []*v1beta2.TEndpoint, err error) {
...
// Get retrieves the TEndpoint from the indexer for a given namespace and name.
func (s tEndpointNamespaceLister) Get(name string) (*v1beta2.TEndpoint, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta2.Resource("tendpoint"), name)
	}
	return obj.(*v1beta2.TEndpoint), nil
}
