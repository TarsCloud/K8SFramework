...
// List lists all TFrameworkConfigs in the indexer.
func (s *tFrameworkConfigLister) List(selector labels.Selector) (ret []*v1beta2.TFrameworkConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta2.TFrameworkConfig))
	})
	return ret, err
}
...
// List lists all TFrameworkConfigs in the indexer for a given namespace.
func (s tFrameworkConfigNamespaceLister) List(selector labels.Selector) (ret []*v1beta2.TFrameworkConfig, err error) {
...
// Get retrieves the TFrameworkConfig from the indexer for a given namespace and name.
func (s tFrameworkConfigNamespaceLister) Get(name string) (*v1beta2.TFrameworkConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta2.Resource("tframeworkconfig"), name)
	}
	return obj.(*v1beta2.TFrameworkConfig), nil
}
