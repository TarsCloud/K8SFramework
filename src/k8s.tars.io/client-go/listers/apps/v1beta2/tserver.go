...
type TServerLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta2.TServer, err error)
...
// List lists all TServers in the indexer.
func (s *tServerLister) List(selector labels.Selector) (ret []*v1beta2.TServer, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta2.TServer))
	})
	return ret, err
}
...
type TServerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta2.TServer, error)
...
// List lists all TServers in the indexer for a given namespace.
func (s tServerNamespaceLister) List(selector labels.Selector) (ret []*v1beta2.TServer, err error) {
...
// Get retrieves the TServer from the indexer for a given namespace and name.
func (s tServerNamespaceLister) Get(name string) (*v1beta2.TServer, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta2.Resource("tserver"), name)
	}
	return obj.(*v1beta2.TServer), nil
}
