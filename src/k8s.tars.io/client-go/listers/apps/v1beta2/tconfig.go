...
// List lists all TConfigs in the indexer.
func (s *tConfigLister) List(selector labels.Selector) (ret []*v1beta2.TConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta2.TConfig))
	})
	return ret, err
}
...
// List lists all TConfigs in the indexer for a given namespace.
func (s tConfigNamespaceLister) List(selector labels.Selector) (ret []*v1beta2.TConfig, err error) {
...
// Get retrieves the TConfig from the indexer for a given namespace and name.
func (s tConfigNamespaceLister) Get(name string) (*v1beta2.TConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta2.Resource("tconfig"), name)
	}
	return obj.(*v1beta2.TConfig), nil
}
