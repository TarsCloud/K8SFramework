...
// List lists all TImages in the indexer.
func (s *tImageLister) List(selector labels.Selector) (ret []*v1beta3.TImage, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta3.TImage))
	})
	return ret, err
}
...
// List lists all TImages in the indexer for a given namespace.
func (s tImageNamespaceLister) List(selector labels.Selector) (ret []*v1beta3.TImage, err error) {
...
// Get retrieves the TImage from the indexer for a given namespace and name.
func (s tImageNamespaceLister) Get(name string) (*v1beta3.TImage, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta3.Resource("timage"), name)
	}
	return obj.(*v1beta3.TImage), nil
}
