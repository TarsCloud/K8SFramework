...
// List lists all TTrees in the indexer.
func (s *tTreeLister) List(selector labels.Selector) (ret []*v1beta3.TTree, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta3.TTree))
	})
	return ret, err
}
...
// List lists all TTrees in the indexer for a given namespace.
func (s tTreeNamespaceLister) List(selector labels.Selector) (ret []*v1beta3.TTree, err error) {
...
// Get retrieves the TTree from the indexer for a given namespace and name.
func (s tTreeNamespaceLister) Get(name string) (*v1beta3.TTree, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta3.Resource("ttree"), name)
	}
	return obj.(*v1beta3.TTree), nil
}
