...
type TConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta3.TConfig, err error)
...
// List lists all TConfigs in the indexer.
func (s *tConfigLister) List(selector labels.Selector) (ret []*v1beta3.TConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta3.TConfig))
	})
	return ret, err
}
...
type TConfigNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta3.TConfig, error)
...
// List lists all TConfigs in the indexer for a given namespace.
func (s tConfigNamespaceLister) List(selector labels.Selector) (ret []*v1beta3.TConfig, err error) {
...
// Get retrieves the TConfig from the indexer for a given namespace and name.
func (s tConfigNamespaceLister) Get(name string) (*v1beta3.TConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta3.Resource("tconfig"), name)
	}
	return obj.(*v1beta3.TConfig), nil
}
