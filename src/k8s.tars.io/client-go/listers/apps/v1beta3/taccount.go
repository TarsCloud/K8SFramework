...
type TAccountLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta3.TAccount, err error)
...
// List lists all TAccounts in the indexer.
func (s *tAccountLister) List(selector labels.Selector) (ret []*v1beta3.TAccount, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta3.TAccount))
	})
	return ret, err
}
...
type TAccountNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta3.TAccount, error)
...
// List lists all TAccounts in the indexer for a given namespace.
func (s tAccountNamespaceLister) List(selector labels.Selector) (ret []*v1beta3.TAccount, err error) {
...
// Get retrieves the TAccount from the indexer for a given namespace and name.
func (s tAccountNamespaceLister) Get(name string) (*v1beta3.TAccount, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta3.Resource("taccount"), name)
	}
	return obj.(*v1beta3.TAccount), nil
}
