...
func (r *TFrameworkConfig) GetDefaultNodeImage(namespace string) (image string, secret string) {
	var tfc *tarsAppsV1beta3.TFrameworkConfig
	...
	tfc, _ = tarsClient.AppsV1beta3().TFrameworkConfigs(namespace).Get(context.TODO(), tarsMeta.FixedTFrameworkConfigResourceName, k8sMetaV1.GetOptions{})
	if tfc != nil {
		return tfc.NodeImage.Image, tfc.NodeImage.Secret
	}
...
func (r *TFrameworkConfig) GetTFrameworkConfig(namespace string) *tarsAppsV1beta3.TFrameworkConfig {
	tfc, _ := tfcMap.Load(namespace)
	if tfc != nil {
		return tfc.(*tarsAppsV1beta3.TFrameworkConfig)
	}
	return nil
}

func (r *TFrameworkConfig) setTFrameworkConfig(tfc *tarsAppsV1beta3.TFrameworkConfig) {
...
func (r *TFrameworkConfig) setupTFCWatch(factories *InformerFactories) {
	tfcInformer := factories.TarsInformerFactory.Apps().V1beta3().TFrameworkConfigs()
	tfcInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(oldObj, newObj interface{}) {
			newTfc := newObj.(*tarsAppsV1beta3.TFrameworkConfig)
			oldTfc := oldObj.(*tarsAppsV1beta3.TFrameworkConfig)
